...
<nav class="navbar navbar-expand-lg navbar-light bg-primary">
    <a class="btn btn-primary" >已打分的周报</a>|
...
<div align="center">
    ...
    <c:if test="${pageInfo.hasPreviousPage eq true}">
        <a href="/AllWeekly?method=mark">首页</a>|<a href="/AllWeekly?pageNum=${pageInfo.prePage}&method=mark">上一页</a>|
    </c:if>
    <c:forEach begin="1" end="${pageInfo.pages}" var="i">
        ...
        <c:if test="${i != pageInfo.pageNum }">
            <a href="/AllWeekly?pageNum=${i }&method=mark">${i }</a>
        </c:if>
    </c:forEach>
    <c:if test="${pageInfo.hasNextPage eq true}">
        |<a href="AllWeekly?pageNum=${pageInfo.nextPage}&method=mark">下一页</a>|<a href="AllWeekly?pageNum=${pageInfo.lastPage}&method=mark">尾页</a>
    </c:if>
